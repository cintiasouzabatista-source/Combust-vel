...
function renderAbast() {
    ...
    if (!abastecimentos.length) {
        ...
        ultimos.innerHTML = '<p class="muted">Sem dados</p>';
        return;
    }
...
function calcularKPIs() {
    ...
    document.getElementById('mediaGeral').textContent = `${mediaGeral} km/L`;
    ...
    if (trocaOleo) {
        ...
        document.getElementById('proxOleo').textContent = faltam > 0? `${faltam} km` : 'VENCIDA';
        document.getElementById('cardOleo').className = faltam <= 0? 'card alerta' : 'card';
    }
}
