...
document.addEventListener('DOMContentLoaded', () => {
    ...
    renderManut();
    calcularKPIs();
    // Inicia na aba painel
    trocarAba('painel');
});

// ===== TABS (Corrigido: não depende de evento de clique direto) =====
...
function calcularKPIs() {
    ...
    const kmAtualEl = document.getElementById('kmAtual');
    const mediaGeralEl = document.getElementById('mediaGeral');
    const proxOleo = document.getElementById('proxOleo');
    const cardOleo = document.getElementById('cardOleo');

    const total = abastecimentos.reduce((s, a) => s + a.total, 0);
    if (totalGasto) totalGasto.textContent = `R$ ${total.toFixed(2)}`;

    const kmAtual = abastecimentos.length ? abastecimentos[abastecimentos.length-1].km : 0;
    if (kmAtualEl) kmAtualEl.textContent = `${kmAtual} km`;
    ...
    if (mediaGeralEl) mediaGeralEl.textContent = `${mediaGeral} km/L`;

    const trocaOleo = manutencoes.filter(m => m.nome === 'Óleo Motor')[0];
    if (trocaOleo && proxOleo) {
        const prox = trocaOleo.km + trocaOleo.intervalo;
        const faltam = prox - kmAtual;
        proxOleo.textContent = faltam > 0 ? `${faltam} km` : 'VENCIDA';
        if (cardOleo) cardOleo.className = faltam <= 0 ? 'card alerta' : 'card';
    }
}
